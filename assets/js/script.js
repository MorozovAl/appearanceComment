...
window.onload = function dateComment() {
    let commentDate = document.getElementsByClassName("vk-comment-date");
    let commentDateReverse = [];
    commentDateReverse.length = commentDate.length;
    for (let x = 0; x < commentDate.length; x++) {
        commentDateReverse[x] = commentDate[commentDate.length - x - 1];
    }

    function innerCommentDate(arrs) {
        let count = 0;
        for (let i = 0; i < arrs.length; i++) {
            ...
            if (myDate - count > 0) {
                arrs[i].innerHTML = `${myDate - count} ${months[month - 1]} ${hours >= 10 ? hours : '0' + hours}:${min >= 10 ? min : '0' + min}`;
            }

            if (myDate - count == 0) {
                arrs[i].innerHTML = `${myDate - count + 1} ${months[month - 1]} ${hours >= 10 ? hours : '0' + hours}:${min >= 10 ? min : '0' + min}`;
            }

            if (myDate - count < 0) {
                arrs[i].innerHTML = `${30 - 1} ${months[month - 2]} ${hours >= 10 ? hours : '0' + hours}:${min >= 10 ? min : '0' + min}`;
            }
        }
    }
    innerCommentDate(commentDateReverse);
};
